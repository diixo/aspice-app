...
class TrackerItem:
	...
	def _load(self, data: dict[str, Any] = None):
		...
		# TODO: These need to match the priority and status method from self._fields to get the extra information about the field and not just the id, name, and type that is provided on the item endpoint.

		# This is because these fields have extra information that is needed to be able to update them properly.
		# For example, the priority field has a list of possible values that is needed to be able to update the field since it is a choice field.
		# The status field has a list of possible values as well as some extra information about the workflow that is needed to be able to update the field properly.
		# So, these will need to get the field from the cached fields on load and then set the value on that field to a ChoiceValue instantiated from the given information.
