...
class Codebeamer:
	...
	def _get_project_by_id(self, id: int) -> Project | None:
		try:
			resp = self._client.get(f'projects/{id}')
			project = Project(**resp, client=self._client)
			return project
		except:
			return
	...
	def get_tracker_types(self) -> dict[str, Any]:
		"""Fetches all available tracker types.

		Returns:
		dict[str, int] — Available map of tracker types onto id's from `GET /trackers/types`.
		"""
		tracker_types = self._client.get('trackers/types')
		if isinstance(tracker_types, list):
			return { t['name']: t['id'] for t in tracker_types }
		return {}
		
	def _get_tracker_by_id(self, id: int) -> Tracker | None:
		try:
			response = self._client.get(f'trackers/{id}')
			tracker = Tracker(**response, client=self._client)
			return tracker
	...
	def get_tracker_item(self, id: int) -> TrackerItem | None:
		...
		try:
			response = self._client.get(f'items/{id}')
			item = TrackerItem(**response, client=self._client)
			return item
		except Exception as e:
			logger.exception(e)
			return
	...
	def get_item_typemap(self):
		return TrackerItem.TYPE_MAP
