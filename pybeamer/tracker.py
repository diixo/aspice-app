...
class Tracker:
	...
	if TYPE_CHECKING:
		...
		_type: dict[str, Any] | None # TODO: TrackerTypeReference
		_type_id: int
		_type_name: str | None
		_deleted: bool | None
		_hidden: bool | None
	...
	@property
	@loadable
	def type(self) -> dict[str, Any] | None: # TODO: TrackerTypeReference
		"""The type-reference of tracker."""
		return self._type

	@property
	@loadable
	def type_id(self) -> int:
		"""The ID of the tracker's type."""
		return self._type_id

	@property
	@loadable
	def type_name(self) -> str | None:
		"""The name of the tracker's type."""
	...
	def _load(self, data: dict[str, Any] = None):
		...
		self._created_at = datetime.strptime(created_at, '%Y-%m-%dT%H:%M:%S.%f') if created_at else None
		...
		if type_info and isinstance(type_info, dict):
			...
			self._type_name = type_info.get('name')
			self._type_id = type_info.get('id', -1)
		else:
			raise ValueError(f"Expected \"type\" field in tracker-data to be a dict, but got {type(type_info)}")
	...
	def create_tracker_item(
		...
		**kwargs,
		...
		if type_name in TrackerItem.TYPE_MAP:
			data['typeName'] = type_name

			data['categories'] = [{'id': TrackerItem.TYPE_MAP[type_name], 'name': type_name, 'type': 'ChoiceOptionReference'}]
		else:
			raise ValueError(f"Invalid type_name: {type_name}. Must be one of: {', '.join(TrackerItem.TYPE_MAP.keys())}")
	...
	def delete(self) -> bool:
		"""Deletes the tracker."""
		try:
			self._client.delete(f'trackers/{self.id}')
			self._deleted = True
			self._id = -1
			return True
		except Exception as e:
	...
	def get_permissions(self)->list:
		"""Fetches the permissions for the tracker."""
		try:
			return self._client.get(f'trackers/{self.id}/permissions')
		except Exception as e:
			logger.exception(e)
			return []